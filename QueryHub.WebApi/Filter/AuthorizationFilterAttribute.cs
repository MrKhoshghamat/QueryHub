...
namespace QueryHub.WebApi.Filter
{
    public class AuthorizationFilterAttribute : Attribute, IAuthorizationFilter
    {
        ...
        public AuthorizationFilterAttribute(IConfiguration configuration, string apiKey, string apiKeySecondary)
        {
        ...
        public void OnAuthorization(AuthorizationFilterContext context)
        {
            ...
            if (apiKeyHeader.Any())
            {
                ...
                {
                    keys.AddRange(collection: _apiKeySecondary?.Split(',') ?? Array.Empty<string>());
                }

                if (keys.FindIndex(x => x != null && x.Equals(apiKeyHeader, StringComparison.OrdinalIgnoreCase)) == -1)
                {
